...
class TimeSlot():
    ...
    def __init__(self, week_day, starts_at, ends_at):
        '''Initializes an object of the TimeSlot class.
            Atributes like ends_at and starts_at must be in HH:MM:SS format'''
    ...
    def from_str_repr(cls, repr):
        '''Recieves a string fromatted as form WEEKDAY HH:MM:SS-HH:MM:SS
            and creates the TimeSlot object according to that data'''
        info = repr.split()
        week_day = info[0]
    ...
    def __eq__(self, other):
        ...
        if all(self._week_day == other.get_week_day(), self._starts_at == other.get_starts_at(), self._ends_at == other.get_ends_at):
            return True
        else:
            return False
